...
outf=os.path.join(outfolder,"%s_out_%d.txt"%(dataf.replace("../","").replace(".txt",""),jid))
print(outf)
outdata=dict()

outdata["ratesdif"]=ratesdif_str

mediandata_subset=np.loadtxt(dataf)
allgroups_subset=list(map(str,np.arange(len(mediandata_subset)))) #dummy
